...
```
En el microservicio que estoy defendiendo, la regla de negocio principal es que no puede existir un examen sin una atención asociada. Por eso, antes de guardar un examen, valido mediante Feign que el atencionId exista en atencion-service. Si no existe, el examen no se persiste.
Antes de persistir un examen, el microservicio valida mediante Feign que la atención exista. Si no existe o el servicio remoto falla, el registro no se guarda.


POST  http://localhost:8087/api/examenes

GET VER TODOS  http://localhost:8087/api/examenes

PUT MODIFICAR EXAMEN http://localhost:8087/api/examenes/ID_AQUI

post para mostrar error de que no se puede pedir examen sin una previa atencion

{
  "nombreExamen": "Resonancia",
  "resultado": "Pendiente",
  "fechaExamen": "2026-05-22",
  "atencionId": 999
}

POST  CREAR EXAMEN http://localhost:8087/api/examenes
ahora uno valido
{
  "nombreExamen": "Resonancia",
  "resultado": "Pendiente",
...
SELECT * FROM examen_db.examenes;          para consultar bs

GET http://localhost:8087/api/examenes/1/detalle     muestra dto feing comunicacion entre microservicios relacion logica
---
